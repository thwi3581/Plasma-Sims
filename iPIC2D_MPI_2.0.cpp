...
void Poisson_solver(std::vector<std::vector<double>>& phi, const std::vector<std::vector<double>>& rho, 
    int NGx_domain, int NGy_domain, double dx, double dy, int left, int right, int up, int down, int size,
    MPI_Comm cart_comm, int max_iteration = 1000, double tol = 1e-6) {
...
int main(int argc, char** argv) {
    ...
    double VT_e = sqrt(2.0 * T / m_e);
    double VT_i = sqrt(2.0 * T / m_i);
    ...
    for (int i = 0; i < Ne_domain; ++i) {
        ...
        e.px = uniform_x(gen);
        e.py = uniform_y(gen);
        e.vx = (VT_e / std::sqrt(2.0)) * normal_dist(gen);
        e.vy = (VT_e / std::sqrt(2.0)) * normal_dist(gen);
    ...
    for (int i = 0; i < Ni_domain; ++i) {
        ...
        ion.vx = (VT_i / std::sqrt(2.0)) * normal_dist(gen);
        ion.vy = (VT_i / std::sqrt(2.0)) * normal_dist(gen);
        ions.push_back(ion);
    }

    for (size_t i = 0; i < electrons.size(); ++i) {
        electrons[i].pm = 1 - 2 * (i % 2);
        electrons[i].vx = electrons[i].vx + (V0e * electrons[i].pm);
    ...
    for (size_t i = 0; i < ions.size(); ++i) {
        ions[i].pm = 1 - 2 * (i % 2);
        ions[i].vx = ions[i].vx + (V0i * ions[i].pm);
    ...
    Poisson_solver(phi, rho, NGx_domain, NGy_domain, dx, dy, left, right, up, down, size, cart_comm);
    ...
    int electrons_on_sphere = 0; 
    int ions_on_sphere = 0;
    ...
    for (int t = 0; t < NT; ++t) {
        ...
        int global_electrons_lost = 0;
        MPI_Reduce(&total_electrons_lost, &global_electrons_lost, 1, MPI_INT, MPI_SUM, 0, cart_comm);
        MPI_Bcast(&global_electrons_lost, 1, MPI_INT, 0, MPI_COMM_WORLD);

        int global_ions_lost = 0;
        MPI_Reduce(&total_ions_lost, &global_ions_lost, 1, MPI_INT, MPI_SUM, 0, cart_comm);
        MPI_Bcast(&global_ions_lost, 1, MPI_INT, 0, MPI_COMM_WORLD);

        if (size > 1) {
            if (size == 4) {
                if (left == MPI_PROC_NULL) {
                    for (int i = 0; i < global_electrons_lost / 2; ++i) {
                        Particle e; 
                        e.px = 0.0; 
                        e.py = uniform_y(gen); 
                        e.vx = V0e * normal_dist(gen); 
                        e.vy = normal_dist(gen);
                        e.pm = 1;
                        electrons.push_back(e);
                    }
                }
                ...
                if (left == MPI_PROC_NULL) {
                    for (int i = 0; i < global_electrons_lost; ++i) {
                        Particle e; 
                        e.px = 0.0; 
                        e.py = uniform_y(gen); 
                        e.vx = V0e * normal_dist(gen); 
        ...
        if (size > 1) {
            if (size == 4) { 
                if (left == MPI_PROC_NULL) {
                    for (int i = 0; i < global_ions_lost / 2; ++i) {
                        Particle ion; 
                        ion.px = 0.0; 
                        ion.py = uniform_y(gen); 
                        ion.vx = V0i * normal_dist(gen); 
                        ion.vy = 0.0;
                        ion.pm = 1;
                        ions.push_back(ion);
                    }
                ...
                if (left == MPI_PROC_NULL) {
                    for (int i = 0; i < global_ions_lost; ++i) {
                        Particle ion; 
                        ion.px = 0.0; 
                        ion.py = uniform_y(gen); 
                        ion.vx = V0i * normal_dist(gen); 
                        ion.vy = normal_dist(gen);
                        ion.pm = 1;
                        ions.push_back(ion);
                    }
                }
        ...
        //std::cout << "# of electrons: " << electrons.size() << std::endl;
        //std::cout << "# of ions: " << ions.size() << std::endl;
    ...
    int total_electrons_on_sphere = 0; 
    int total_ions_on_sphere = 0; 

    MPI_Reduce(&local_charge, &total_charge, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
    MPI_Reduce(&electrons_on_sphere, &total_electrons_on_sphere, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
    MPI_Reduce(&ions_on_sphere, &total_ions_on_sphere, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);

    auto end = std::chrono::high_resolution_clock::now();

    if (rank == 0) std::cout << "Total sphere charge: " << total_charge << std::endl;
    if (rank == 0) std::cout << "Total sphere electrons: " << total_electrons_on_sphere << std::endl;
    if (rank == 0) std::cout << "Total sphere ions: " << total_ions_on_sphere << std::endl;

    MPI_Finalize();
    std::chrono::duration<double> elapsed = end - start;
    if (rank == 0) std::cout << "Total simulation time: " << elapsed.count() << " seconds" << std::endl;
    return 0;
}
